...
The guassian proccess model allows for data that is correlated in the response. We assume that that the power bills are correlated with a first-order autoregressive structure as shown below:
$$
\sigma^2
\begin{bmatrix}
